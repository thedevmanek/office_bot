#!/usr/bin/env python3
"""Start a tmux session for researcher-visible OpenHRI runs."""
...
def main(argv=None):
    ...
    if not shutil.which("tmux"):
        print(
            "researcher_session: tmux is not installed on the host. "
            "Install tmux, then rerun make researcher-session.",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        ...
        print(f"researcher_session: {exc}", file=sys.stderr)
        return 2
...
def session_commands(repo_root, args, trial, run_dir, container_run_dir):
    ...
    return {
        ...
        "artifacts": artifacts_command(repo_root, run_dir, event_log, manifest, summary),
...
def controls_command(repo_root, args, trial):
    lines = [
        f"printf '%s\\n' {shlex.quote('OpenHRI researcher session')}",
        ...
        "printf '%s\\n' '  bottom-right  artifacts: live run folder/events/manifest preview'",
        "printf '\\n%s\\n' 'Extra windows:'",
        ...
        "printf '%s\\n' '  Keyboard: Ctrl-b then [, use arrows/PageUp/PageDown, q to exit scroll mode'",
        "printf '\\n%s\\n' 'Rerunning commands:'",
        ...
        "printf '%s\\n' '  From outside tmux: make researcher-stop'",
        "printf '\\n%s\\n' 'Detach without stopping:'",
        "printf '%s\\n' '  Ctrl-b then d'",
        "printf '\\n%s\\n' 'This controls window is a normal shell. You can run make researcher-stop here.'",
        "exec bash -i",
    ]
...
def artifacts_command(repo_root, run_dir, event_log, manifest, summary):
    lines = [
    ...
    if rerun_command:
        ...
        script.extend(
            [
                ...
                'status=$?',
                'set -e',
                'printf "\\nCommand exited with status %s. This pane stays open.\\n" "$status"',
                'printf "%s\\n" "Run rerun to execute it again, or press Up then Enter to edit/rerun."',
...
def start_session(session, commands, repo_root, container, attach=True):
    ...
    artifacts_pane = split_window(container_pane, "-v", commands["artifacts"])
    for pane, title in (
        (sim_pane, "simulation"),
        (trial_pane, "detector"),
        (container_pane, "container"),
        (artifacts_pane, "artifacts"),
    ):
...
def configure_tmux_session(session, repo_root):
    ...
    commands = [
        ...
        [
            ...
            f"run-shell 'make -C {shlex.quote(str(repo_root))} researcher-stop'",
        ],
        [
            "tmux",
            "bind-key",
